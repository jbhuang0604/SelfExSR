	...
	bool QPBO<REAL>::Probe(int* mapping)
{
	...
	while ( 1 )
	{
		...
		for (i_index=list.GetNext(-current_list) ; i_index>=0; i_index=i_index_next)
		{
			...
			maxflow(true, true);
			code_assert(what_segment(i)==0 && what_segment(_i[1])==1);
			if (!(what_segment(i)==0 && what_segment(_i[1])==1))
				mexErrMsgTxt("Probe can't fix nodes - overflow likely.");
			...
			maxflow(true, true);
			code_assert(what_segment(i)==1 && what_segment(_i[1])==0);
	...
	void QPBO<REAL>::Probe(int* mapping, ProbeOptions& options)
{
    ...
    int* mapping1;
	if (probe_options.iters > 1)
		mapping1 = new int[GetNodeNum()];
	...
	while ( 1 )
	{
		...
		if ( ++iter >= probe_options.iters || user_terminated )
			break;
	...
	bool QPBO<REAL>::Improve(int N, int* order_array, int* fixed_nodes)
{
	...
	for (i=nodes[0]; i<node_last[0]; i++)
	{
		...
		else if (i->label != i->user_label) 
		{
			success = true;
			i->user_label = i->label;
		}
	}
